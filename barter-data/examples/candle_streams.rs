...
async fn main() {
    ...
    // Initialise 1m Candles Streams for BinanceFuturesUsd only
    // '--> each call to StreamBuilder::subscribe() creates a separate WebSocket connection
    let streams = Streams::<Candles>::builder()

        // Separate WebSocket connection for BTC_USDT stream
        .subscribe([
            (BinanceFuturesUsd::default(), "btc", "usdt", MarketDataInstrumentKind::Perpetual, Candles),
        ])

        // Separate WebSocket connection for ETH_USDT stream
        .subscribe([
        ...
        .subscribe([
            (BinanceFuturesUsd::default(), "xrp", "usdt", MarketDataInstrumentKind::Perpetual, Candles),
            (BinanceFuturesUsd::default(), "sol", "usdt", MarketDataInstrumentKind::Perpetual, Candles),
        ])
        .init()
        .await
        .unwrap();
    ...
    while let Some(event) = joined_stream.next().await {
        info!(?event, "Binance USD-M Futures 1m candle");
    }
}
